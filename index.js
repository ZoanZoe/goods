...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('== BoBOT ==', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('ArugaZ x BoBOT', 'yellow'))
	...
	await aruga.sendText(chat.id, `Nuwun sewu, klompok ing bot iki kebak\nMax Group yaiku: ${groupLimit}`).then(() => {
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
	    ...
	    await aruga.sendText(chat.id, `Nuwun sewu, BOT metu yen anggota grup ora ngluwihi ${memberLimit} wong`).then(() => {
	      aruga.leaveGroup(chat.id)
	      aruga.deleteChat(chat.id)
	    })
	    } else {
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai, Aku BoBOT. Kanggo ngerteni prentah ing jinis bot iki ${prefix}menu`)
        })
	    }
	}
    })
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host && isWelcome) {
            ...
            await aruga.sendTextWithMentions(event.chat, `Halo, Sugeng rawuh ing grup iki @${event.who.replace('@c.us', '')} \n\nSeneng-seneng karo kita✨`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
			await aruga.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
            await aruga.sendTextWithMentions(event.chat, `Selamat tinggal @${event.who.replace('@c.us', '')}, Kita bakal kangen sampeyan✨`)
        }
    })

    aruga.onIncomingCall(async (callData) => {
        // ketika seseorang menelpon nomor bot akan mengirim pesan
        await aruga.sendText(callData.peerJid, 'Maaf lagi ora bisa nampa panggilan.\n\n-bot')
        .then(async () => {
