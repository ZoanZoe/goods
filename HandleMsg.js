...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
        ...
        case 'help':
            ...
            break
        case 'menuadmin':
            if (!isGroupMsg) return aruga.reply(from, 'Nuwun sewu, prentah iki mung bisa digunakake ing grup!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal, printah iki mung bisa digunakake dening admin grup!', id)
        ...
        case 'ownerbot':
            ...
            .then(() => aruga.sendText(from, 'Yen sampeyan pengin njaluk fitur, mangga ngobrol karo nomer pamilik!'))
            break
        case 'join':
            if (args.length == 0) return aruga.reply(from, `Yen sampeyan pengin ngundang bot menyang grup, mangga ngajak utawa nganggo\nketik ${prefix}join [link group]`, id)
            let linkgrup = body.slice(6)
            let islink = linkgrup.match(/(https:\/\/chat.whatsapp.com)/gi)
            let chekgrup = await aruga.inviteInfo(linkgrup)
            if (!islink) return aruga.reply(from, 'Nuwun sewu, tautan grup salah! tulung kirim link sing bener', id)
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await aruga.sendText(from, 'Sukses gabung karo grup liwat link!')
                          await aruga.sendText(chekgrup.id, `Hai, Aku BoBOT. Kanggo ngerteni prentah ing jinis bot iki ${prefix}menu`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `Nuwun sewu, klompok ing bot iki kebak\nMax Group yaiku: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return aruga.reply(from, `Nuwun sewu, BOT ora bakal melu yen anggota klompok ora ngluwihi ${memberLimit} wong`, id)
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await aruga.reply(from, 'Sukses gabung karo grup liwat link!', id)
                      })
                      .catch(() => {
                          aruga.reply(from, 'GAISO!!', id)
                      })
            }
            break
        case 'botstat': {
	...
	case 'dewabatch':
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *BoBOT Broadcast* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *BoBOT Broadcas* 〙\n\n${msg}`)
            }
